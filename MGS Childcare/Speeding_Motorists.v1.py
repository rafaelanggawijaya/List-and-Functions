...
while True:
    ...
    if answer == "y":
        ...
        number_offenders += 1
        offenders.append([number_offenders, speeder_name, speeder_speed,
                          fined])
        total_fines += 1
        print("")
    elif answer == "n":
...
total_amt_fines = sum(offender[3] for offender in offenders)
print(f"Total fines:{total_fines}\n")
for offender in offenders:
    print(f"{offender[0]}) {offender[1]} Amount over the limi"
          f"t:{offender[2]}")
print(f"The total amount fined was ${total_amt_fines}")
